...
public class DownloadUtils extends Thread {
	...
	public static boolean fileExists(String file) {
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(new URL("http://launcher.theminecraftian.com/FTB2/" + file).openStream()));
			return !reader.readLine().toLowerCase().contains("not found");
		} catch (Exception e) {
			return false;
		}
	...
	@Override
	public void run() {
		downloadServers.put("Automatic", "launcher.theminecraftian.com");
		BufferedReader in = null;
		// New Servers
		try {
			in = new BufferedReader(new InputStreamReader(new URL("http://launcher.theminecraftian.com/edges.json").openStream()));
			String line;
		...
		// Backup md5 servers
		try {
			in = new BufferedReader(new InputStreamReader(new URL("http://launcher.theminecraftian.com/mirrors").openStream()));
			String line;
